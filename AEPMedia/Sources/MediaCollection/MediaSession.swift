...
class MediaSession {
    ...
    func queue(hit: MediaHit) {
        guard isSessionActive else {
            Log.debug(label: Self.LOG_TAG, "[\(Self.CLASS_NAME)<\(#function)>] - Unable to queue hit. Media Session is inactive.")
            return
        }
    ...
    func end(onSessionEnd sessionEndHandler: (() -> Void)? = nil) {
        guard isSessionActive else {
            Log.debug(label: Self.LOG_TAG, "[\(Self.CLASS_NAME)<\(#function)>] - Unable to end session. Session (\(id)) is inactive")
            return
        }
    ...
    func abort(onSessionEnd sessionEndHandler: (() -> Void)? = nil) {
        guard isSessionActive else {
            Log.debug(label: Self.LOG_TAG, "[\(Self.CLASS_NAME)<\(#function)>] - Unable to abort session. Session (\(id)) is inactive")
            return
        }
    ...
    func handleMediaStateUpdate() {
        Log.warning(label: Self.LOG_TAG, "[\(Self.CLASS_NAME)<\(#function)>] - This function should be handle by the child class.")
    }
